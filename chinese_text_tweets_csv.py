...
from tweetybird import tweet_full_text, tweet_user_mentions, tweet_date, TWITTER_DATE_FORMAT


def from_twitter_json_file(filepath: str) -> Generator[Tuple[str, str, str], None, None]:
    with open("chinese_text_tweet.json", "r") as f_in:
        tweets = json.load(f_in)
        for tweet in tweets:
            dt_tweet = datetime.datetime.strptime(tweet_date(tweet), TWITTER_DATE_FORMAT).strftime("%Y-%m-%d")
            yield dt_tweet, tweet_user_mentions(tweet), tweet_full_text(tweet)
...
def tweets_simplified_csv(summary_tweets: Generator[Tuple[str, str, str], None, None], csv_filepath: str):
    ...
    with open(csv_filepath, "w") as f_out:
        csv_wrtr = csv.DictWriter(f_out, fieldnames=("Date", "Source", "Tweet"), quoting=csv.QUOTE_ALL)
        csv_wrtr.writeheader()

        for tweet_date, source, tweet_text in sorted_tweets:
            csv_wrtr.writerow({"Date": tweet_date, "Source": source, "Tweet": tweet_text })
